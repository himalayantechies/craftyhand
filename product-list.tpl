				...
				{if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.minimal_quantity <= 1 && 					$product.customizable != 2 && !$PS_CATALOG_MODE}
					{if ($product.allow_oosp || $product.quantity > 0)}
						{if isset($static_token)}
							<a  class="ajax_add_to_cart_button exclusive btn_add_cart" style="display:none;" rel="ajax_id_product_{$product.id_product|intval}" href="{$link->getPageLink('cart',false, NULL, "add&amp;id_product={$product.id_product|intval}&amp;token={$static_token}", false)}" title="{l s='Add to cart'}" style="float:right;margin-top:16px;">
                            <i class="icon-shopping-cart"></i> 
                           </a> 
						{else}
						
							<a class="ajax_add_to_cart_button exclusive btn_add_cart" rel="ajax_id_product_{$product.id_product|intval}" 
...
$('li.ajax_block_product').hover(function() {
	$('a.ajax_add_to_cart_button').show();
});
