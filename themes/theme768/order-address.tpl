...
// <![CDATA[
	...
	{
		return {
						'invoice': "{l s='Billing' js=1}",
						'delivery': "{l s='Shipping address' js=1}"
			};
	...
	function buildAddressBlock(id_address, address_type, dest_comp)
	{
		...
		li_content['update'] = '<a style="background:none; color:#003399; border:none; text-shadow:none;margin:0 0 0 -10px;" href="{$link->getPageLink('address', true, NULL, "id_address") }'+id_address+'&amp;back={$back_order_page}?step=1{if $back}&mod={$back}{/if}" title="{l s='Change' js=1}">&raquo; {l s='Change' js=1}</a>'; 
...
<!--{if !$opc}<h1><span>{l s='Shipping & Billing'}</span></h1>{else}<h2><span>1</span> {l s='Addresses'}</h2>{/if} -->
...
{if !$opc}<h1 style="font-size:24px;font-weight:normal;margin:23px 0 -12px;font-family:arial;"><span>{l s='Shipping & Billing'}</span></h1>{else}<h2><span>1</span> {l s='Addresses'}</h2>{/if}

	<div class="addresses clearfix">
		<div style="display:inline-block;padding-top:51px; width:100%">
			<div style="float:left;overflow:hide;padding-right:10px;">
				<label style="font-size:13px;font-weight:bold;color:#585858!important;" for="id_address_delivery">{if $cart->isVirtualCart()}{l s='Choose a billing address:'}{else}{l s='Choose a shipping address:'}{/if}</label>
			</div>
			<div style="float:left;overflow:hide;width:100%">
				...
				<p class="cart_navigation submit clearfix" style="padding-top:10px;margin:-117px 0 0;">
		<input type="hidden" class="hidden" name="step" value="2" />
		<input type="hidden" name="back" value="{$back}" />
		
		<input type="submit" name="processAddress" value="{l s='Proceed to Carrier Selection '} &raquo;" class="btn-success btn" style="float:right; font-size:13px;" />
	</p>
	...
	<div style="display:inline-block;width:100%;padding-left:0px !important;margin:32px 0 0!important;" class="checkbox addressesAreEquals" {if $cart->isVirtualCart()}style="display:none;"{/if}>
		...
		<label for="addressesAreEquals">{l s='Use the shipping address as the billing address.'}</label>
	</div>
	<!--
...
-->
	<div style="display:inline-block;width:100%;">
		...
		{if $addresses|@count > 1}
			<!--<div style="display:inline-block;width:100%">
				<div style="float:left;overflow:hide;padding-right:10px;"> -->
					<label for="id_address_invoice" class="strong" style="float:left;overflow:hidden;padding-right:10px;padding-bottom:10px;"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{l s='Choose a billing address:'}</label>
				<!--</div>-->
				<!--<div style="float:left;overflow:hide;width:70%"> -->
					<select name="id_address_invoice" id="id_address_invoice" class="address_select" onchange="updateAddressesDisplay();{if $opc}updateAddressSelection();{/if}" style="float:left;overflow:hidden; width:48%;">
	...
	-->
		...
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}" title="{l s='Add'}" class="">{l s='Add a new address'}</a>
		</p>
		{if !$opc}
		<div id="ordermsg" class="clearfix">
			<p class="txt" style="margin-bottom:10px;">{l s='If you would like to add a comment about your order, please write it in the field below.'}</p>
...
{if !$opc}
	<p class="cart_navigation submit clearfix" style="padding-top:10px;">
		<input type="hidden" class="hidden" name="step" value="2" />
		<input type="hidden" name="back" value="{$back}" />
		<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")}" title="{l s='Previous'}" class="">&laquo; {l s='Previous'}</a>
		<input type="submit" name="processAddress" value="{l s='Proceed to Carrier Selection '} &raquo;" class="btn-success btn" style="float:right; font-size:13px;" />
	</p>
</form>
{else}
