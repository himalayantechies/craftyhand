...
<!--<tr><td colspan="2">
		<h5><a class="order_overview_product_link" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'htmlall':'UTF-8'}">{$product.name|escape:'htmlall':'UTF-8'}</a></h5> 
...
<tr id="product_{$product.id_product}_{$product.id_product_attribute}_0_{$product.id_address_delivery|intval}{if !empty($product.gift)}_gift{/if}" class="bordercolor {if isset($productLast) && $productLast && (!isset($ignoreProductLast) || !$ignoreProductLast)}last_item{elseif isset($productFirst) && $productFirst}first_item{/if} {if isset($customizedDatas.$productId.$productAttributeId) AND $quantityDisplayed == 0}alternate_item{/if} cart_item address_{$product.id_address_delivery|intval} {if $odd}odd{else}even{/if}">
	...
	<td class="cart_product" style="width=8%;" > 
	...
	<td class="cart"> 
		<h5><a class="order_overview_product_link" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'htmlall':'UTF-8'}"><span  style="font-size:11px;">{$product.name|escape:'htmlall':'UTF-8'}</span></a></h5> 
		
    	{if isset($product.attributes) && $product.attributes}
		<a class="cart-atr"  href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'htmlall':'UTF-8'}"><span  style="font-size:11px;">{$product.attributes|escape:'htmlall':'UTF-8'}</span></a>
		{/if}
		
	<div class="clearfix insset-bottom" style="padding:0px;">
    <!--<span class="title-th" >{l s='Unit price'}:</span> -->
	   <span class="price" style="float:left;font-size:11px;" id="product_price_{$product.id_product}_{$product.id_product_attribute}_{$product.id_address_delivery|intval}{if !empty($product.gift)}_gift{/if}">
	...
	<span class="title-th" style="padding-left: 15px;">{l s='#'}</span> <span style="font-size: 11px;"> {if $product.reference}{$product.reference|escape:'htmlall':'UTF-8'}{else}--{/if}</span>
	</div>

	<div class="clearfix insset-bottom" style="padding:0px;">
        <span class="title-th cart_quantity_title" style="font-size:11px;">{l s='Qty'}: {$product.cart_quantity-$quantityDisplayed}</span>
		...
		{else}
			...
			{if !isset($customizedDatas.$productId.$productAttributeId) OR $quantityDisplayed > 0}
				...
				<!--<div style="float:left;">
					<a rel="nofollow" class="cart_quantity_delete" id="{$product.id_product}_{$product.id_product_attribute}_0_{$product.id_address_delivery|intval}" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery|intval}&amp;token={$token_cart}")}"><i class="icon-trash">Remove</i> Remove</a>
				</div>-->
	...
	{/if}
		...
		<div style="float: right;">
			<span class="title-th"  style="font-size:11px;">{l s=' Subtotal'}:</span>
		...
		<div style="float:left;padding-top:20px;">
					<a rel="nofollow" class="order_overview_product_link" id="{$product.id_product}_{$product.id_product_attribute}_0_{$product.id_address_delivery|intval}" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery|intval}&amp;token={$token_cart}")}">Remove</a>
				</div>
		-->		
    </td>
