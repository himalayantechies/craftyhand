...
<h1 style="padding-top:16px;"><span style="font-size:24px; font-weight:normal;">{l s='Order history'}</span></h1>
...
<div class="block-center" id="block-history">
 ...
 $(function() {
	...
	<table id="order-list" class="std shop_table footable" style="margin:-24px 0 0;text-transform:capitalize;">
		<thead>
			<tr>
		...
		{foreach from=$orders item=order name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				...
				<td class="history_detail">
					...
					<a class="color-myaccount btn btn-info new" style="width:38%;float:left;" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true)}');">{l s='details'}</a>
					{if isset($opc) && $opc}
					
                    <a class="btn btn-info new" style="width:38%;float:right; margin-top:0px !important;"href="{$link->getPageLink('order-opc', true, NULL, "submitReorder&id_order={$order.id_order}")}" title="{l s='Reorder'}">
					{else}
					<a class="btn btn-info new" style="width:38%;float:right; margin-top:0px !important;" href="{$link->getPageLink('order', true, NULL, "submitReorder&id_order={$order.id_order}")}" title="{l s='Reorder'}">
					{/if}
					{l s='Reorder'}
					</a>
