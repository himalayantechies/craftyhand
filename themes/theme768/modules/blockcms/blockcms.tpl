...
<section class="block blockcms_footer span3">
		...
		<ul class="list-footer toggle_content clearfix">
			<!--<li><a href="http://www.craftyhand.com/blog" target="_blank">Blog</a></li>-->
			
			<!--
			{if !$PS_CATALOG_MODE}<li class="first_item"><a href="{$link->getPageLink('prices-drop')}" title="{l s='Price drops' mod='blockcms'}">{l s='Price drops' mod='blockcms'}</a></li>{/if}
			...
			{foreach from=$cmslinks item=cmslink}
				...
				{/if}
			{/foreach}
			<li ><a href="{$link->getPageLink('sitemap')}" title="{l s='sitemap' mod='blockpermanentlinks'}">{l s='sitemap' mod='blockcms'}</a></li>
		</ul>
	{$footer_text}
</section>
