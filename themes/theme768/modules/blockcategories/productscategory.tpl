...
{if count($categoryProducts) > 0 && $categoryProducts !== false}
	<section class="page_product_box blockproductscategory">
		<h3 style="border-top:solid 1px #CCCCCC;"> <span style="font-size:14px;font-weight:bold;"><!--{$categoryProducts|@count}--> {l s='You may also like other products from ' mod='productscategory'} {$category_name} category </span><i class="icon-toggle icon-minus-sign-alt"></i></h3>
			...
			{foreach from=$categoryProducts item='categoryProduct' name=categoryProduct}
                ...
                <div class="clearfix">
                    <a href="{$link->getProductLink($categoryProduct.id_product, $categoryProduct.link_rewrite, $categoryProduct.category, $categoryProduct.ean13)}" class="lnk_img" title="{$categoryProduct.name|htmlspecialchars}"><img src="{$link->getImageLink($categoryProduct.link_rewrite, $categoryProduct.id_image, 'medium_default')}" alt="{$categoryProduct.name|htmlspecialchars}" /></a>
                        ...
                        <a class="product_link noSwipe" style="float:left;" href="{$link->getProductLink($categoryProduct.id_product, $categoryProduct.link_rewrite, $categoryProduct.category, $categoryProduct.ean13)}" title="{$categoryProduct.name|htmlspecialchars}">{$categoryProduct.name|truncate:23:'...'|escape:'htmlall':'UTF-8'}</a>
