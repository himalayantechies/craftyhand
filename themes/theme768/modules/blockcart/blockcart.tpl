...
<section id="cart_block" class="block column_box exclusive">
 ...
 <span class="quantity-formated"><span class="quantity">{$product.cart_quantity}</span>x</span>	
			...
			{$product.name|truncate:13:'...'|escape:html:'UTF-8'}</a>           
 			...
 			<span id="cart_block_combination_of_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}_{$product.id_address_delivery|intval}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
			...
			{if isset($customizedDatas.$productId.$productAttributeId[$product.id_address_delivery])}
				...
				{if !isset($product.attributes_small)}</span>{/if}
			{/if}
