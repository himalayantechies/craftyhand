...
{if !isset($email_create)}
	...
	{/if}-->
	<!--
	<span class="ajax_cart_total{if $cart_qties == 0} hidden {else} hidden {/if}">
		{if $priceDisplay == 1}
			{assign var='blockuser_cart_flag' value='Cart::BOTH_WITHOUT_SHIPPING'|constant}
			{ price=$cart->getOrderTotal(false, $blockuser_cart_flag)}
			{assign var="orderTotal" value =$cart->getordertotal(false)}
			
		{else}
			{assign var='blockuser_cart_flag' value='Cart::BOTH_WITHOUT_SHIPPING'|constant}
			{ price=$cart->getOrderTotal(true, $blockuser_cart_flag)}
			{assign var="orderTotal" value =$cart->getordertotal(false)}
		{/if}
	</span>
	-->
    ...
    <div class="row-fluid">
	<aside id="left_column" class="span1 column">
			
	</aside>
	<script>
	$("#new_account_form_toggle").on("click", function() {
    $("body").scrollDown(0);
});
	</script>
	<form action="{$link->getPageLink('authentication', true)}" method="post" id="create-account_form" class="std span6" style="margin:0 0 -71px 0 !important;">
		<fieldset class="titled_box">
			<h2><span style="font-size:18px;font-weight:normal;color:rgb(97, 154, 12);">{l s='New Customer'}</span></h2>
			<div class="form_content clearfix">
				<p class="title_block">
				{l s='Register to enjoy faster check out and easy access to your order history.'}</p>
				...
				<p class="submit" style="padding-bottom:49px;">
	...
	<!--<span>{l s='No time right now? No problem. You do not need an account to check out.'}</span>-->
	<span class="note">{l s='No time right now? No problem. You do not need an account to check out.'}</span>&nbsp;
	<h1 style="padding-top:31px;"><span id="new_account_form_toggle" class="btn-success btn"style="font-size:13px;font-weight:normal;color:white;margin:-51px 0 0;">{l s='CHECKOUT AS GUEST'}</span></h1>
	</div>
	{/if}
				</p>
				<!--<span class="note">{l s='No time right now? No problem. You do not need an account to check out.'}</span>-->
	...
	<form action="{$link->getPageLink('authentication', true)}" method="post" id="login_form" class="std span6">
		<fieldset class="titled_box">
			<h2><span style="font-size:18px;font-weight:normal;color:rgb(97, 154, 12);">{l s='Returning Customer'}</span></h2>
			<div class="form_content clearfix">
			<p class="title_block">{l s='Sign in to your craftyhand.com account.'}</p>
				<p class="text">
	...
	<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")}" method="post" id="new_account_form" class="std clearfix">
		<fieldset>
			<!--<h1><span class="btn-success btn"style="font-size:13px;font-weight:normal;color:white;">{l s='CHECKOUT AS GUEST'}</span></h1>-->
			<!--<p><span>{l s='No time right now? No problem. You do not need an account to check out.'}</span></p>-->
			<p class="title_block" style="text-align:right;padding-right:10px;"><sup>*</sup> {l s='Indicates required field.'} </p> 
			<div id="opc_account_form" style="display: block; ">
            <div class="shop_box" style="border-bottom:0px; padding-bottom:0px;">
            <div class="row-fluid">
			<!--
			<label for="guest_email">{l s='We require your email address, so that we can communicate with you about your order.'} </label>
				<div class="customspan">
					<div class="customlabel">
						<label for="guest_email"><sup>*</sup> {l s='Email address:'} </label>
					</div>
				...
				<div class="instant_checkout_form">
				<!--<label for="guest_email">{l s='We require your email address, so that we can communicate with you about your order.'} </label> -->
				<table>
				<tr><td class="txtlabel"><label for="guest_email"><sup>*</sup> {l s='Email address:'} </label></td>
					<td >
					<input type="email" class="text" id="guest_email" name="guest_email" value="{if isset($smarty.post.guest_email)}{$smarty.post.guest_email}{/if}" />
					</td>
				</tr>
				<tr><td class="txtlabel"><label for="guest_email"><sup>*</sup> {l s='Confirm Email address:'} </label></td>
					<td >
				...
				<tr><td  class="txtlabel"> <label for="firstname"><sup>*</sup> {l s='Full name:'} </label> </td>
					<td >
					<input type="text" class="text" id="firstname" name="firstname" onblur="$('#customer_firstname').val($(this).val());" value="{if isset($smarty.post.firstname)}{$smarty.post.firstname}{/if}" />
				...
				<!--<tr><td  class="txtlabel"><label for="lastname"><sup>*</sup> {l s='Last name:'}</label></td>
					<td>
					<input type="text" class="text" id="lastname" name="lastname" onblur="$('#customer_lastname').val($(this).val());" value="{if isset($smarty.post.lastname)}{$smarty.post.lastname}{/if}" />
					<input type="hidden" class="text" id="customer_lastname" name="customer_lastname" value="{if isset($smarty.post.lastname)}{$smarty.post.lastname}{/if}" />
					</td>
				...
				-->
					...
					<script>
					$('#new_account_form_toggle').click(function(){
...
<!--                <div class="span6">
				...
				<!--<div class="deli"><span style="margin-top: 10px;">{l s='Your Shipping address'}</span></div>-->
                 <div class="shop_box">
				...
				{foreach from=$dlv_all_fields item=field_name}
                    ...
                    <div class="row-fluid">
					<!-- <div class="span6"> original one-->
					<div class="customspan">
						<!--	
						<div class="customlabel"><label for="company">{l s='Company: '}</label></div>
						<div class="customInput">
						<p class="text">
						<input type="text" class="text" id="company" name="company" placeholder="Leave blank if this is a residence." value="{if isset($smarty.post.company)}{$smarty.post.company}{/if}" />
						</p>
						</div>
						-->
							<!--<p class="text">
						...
						<div class="customlabel required id_state select" >
							<!--<p class="required id_state select" style="text-align:right;"> -->
						...
						<div class="customlabel">
							<label for="phone"><sup>*</sup>&nbsp;{l s='Phone Number:'}{if isset($one_phone_at_least) && $one_phone_at_least}{/if}</label>
						</div>
						<div class="customInput">
...
-->						
				...
				<div class="customlabel" class="required id_state select">
							<p class="required id_state select" style="text-align:right;"> <label for="id_state">&nbsp;{l s='State:'} <sup>*</sup></label></p>
							</div>
							<div class="customInput" class="required id_state select">
							 <p class="required id_state select">
							<select name="id_state" id="id_state">
								<option value="">-</option>
							</select>
					...
					-->
					<!--	<div class="customlabel" class="required id_state select"><label for="id_state">{l s='State'} <sup>*</sup></label></div>
							<div class="customInput" class="required id_state select"><select name="id_state" id="id_state">
								<option value="">-</option>
							</select>
							</div>
					-->
		...
		<p class="cart_navigation clearfix required submit">
			...
			<input type="submit" class="btn-success btn" name="submitGuestAccount" id="submitGuestAccount" value="{l s='Proceed to Checkout'} &raquo;" />
		</p>
	</form>
	{/if}
{else}
...
<form action="{$link->getPageLink('authentication', true)}" method="post" id="account-creation_form" class="std">
	...
	<fieldset class="account_creation titled_box">
		<h2><span>{l s='New to Craftyhand.com? Register below.'}</span></h2>
		<!--<div style="padding-top:10px;" ><sup>*</sup> Indicates required field</div>-->
		...
		<div class="customspan">
			...
			<div class="customInput">
				<p class="required text">
				<input type="email" class="text" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email}{/if}" />			</p>
			</div>
			<!-- added later by Ram -->
			<div class="customlabel">
				<label for="confirm_email">{l s='Confirm Email address:'} </label>
			</div>
			<div class="customInput">
				<p class="required text">
				<input type="email" id="email_validate" name="email_validate" value="{if isset($smarty.post.email_validate)}{$smarty.post.email_validate}{/if}" class="text" />
				</p>
			</div>
			<!-- end of added later by Ram -->	
			<div class="customlabel">
				<label for="passwd">{l s='Password:'} <span class="form_info">{l s='(Five characters minimum)'}</span></label>
			</div>
			<div class="customInput">
				<p class="required password clearfix">
				<input type="password" class="text" name="passwd" id="passwd" /></p>
			</div>
		</div> <!-- end of custom span -->
		<!--<div class="span6">
		<p class="required text">
	...
	<p class="cart_navigation required submit">
		...
		{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'htmlall':'UTF-8'}" />{/if}
		<input style="float:right;"type="submit" name="submitAccount" id="submitAccount" value="{l s='Create account'}" class="btn btn-success save" />
		<!--<span><sup>*</sup>{l s='Required field'}</span>-->
	</p>
</form>
{/if}