...
<li class="ajax_block_product span3 num-{$smarty.foreach.products.iteration}{if $smarty.foreach.products.iteration%$nbItemsPerLine == 0} omega{elseif $smarty.foreach.products.iteration%$nbItemsPerLine == 1} alpha{/if} {if $smarty.foreach.products.iteration > ($smarty.foreach.products.total - $totModulo)}last_line{/if}">
		...
		<div class="center_block" >
            ...
            <div class="clear"></div>
            <h5><a class="product_link" href="{$product.link|escape:'htmlall':'UTF-8'}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|escape:'htmlall':'UTF-8'|truncate:30:'...'}</a></h5>
		...
		<div class="right_block">
			...
			{if (!$PS_CATALOG_MODE AND ((isset($product.show_price) && $product.show_price) || (isset($product.available_for_order) && $product.available_for_order)))}
				{if isset($product.show_price) && $product.show_price && !isset($restricted_country_mode)}
					<span class="price" style="font-weight:bold;font-size:13px;" >{if !$priceDisplay}{convertPrice price=$product.price}{else}{convertPrice price=$product.price_tax_exc}{/if}</span>
				{/if}  
			{/if}	 
				...
				{if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.minimal_quantity <= 1 && 					$product.customizable != 2 && !$PS_CATALOG_MODE}
					{if ($product.allow_oosp || $product.quantity > 0)}
						{if isset($static_token)}
							<a  class="ajax_add_to_cart_button btn-success btn no_hover_product_cart"  rel="ajax_id_product_{$product.id_product|intval}" href="{$link->getPageLink('cart',false, NULL, "add&amp;id_product={$product.id_product|intval}&amp;token={$static_token}", false)}" title="{l s='Add to cart'}" style="float:right;margin-top:10px;padding:0 6px 0 6px;">
                            <!--<i class="icon-shopping-cart"></i> -->
							 <div class="cartbutton" style="font-size:11px;">{l s='Add to cart'}</div>
                           </a> 
						{else}
						
							<a class="ajax_add_to_cart_button btn-success btn no_hover_product_cart" rel="ajax_id_product_{$product.id_product|intval}" 
							href="{$link->getPageLink('cart',false, NULL, "add&amp;id_product={$product.id_product|intval}", false)} title="{l s='Add to cart'}">
                            <span>{l s='Add to cart'}</span>
                            </a>
							
						{/if}						
					{else}
						<span class="btn-success btn no_hover_product_cart" style="float:right;">{l s='Add to cart'}</span>
					{/if}
				{/if}
...
$('ul#product_list').find('li.ajax_block_product').hover(function() {
	if($('ul#product_list').hasClass('grid'))
		$(this).find('a.ajax_add_to_cart_button, span.btn-success btn').removeClass('no_hover_product_cart');
}, function() {
	if($('ul#product_list').hasClass('grid'))
		$(this).find('a.ajax_add_to_cart_button, span.btn-success btn').addClass('no_hover_product_cart');
});
